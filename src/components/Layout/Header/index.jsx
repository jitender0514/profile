    ...
    <header className="z-[99] relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-2 text-[0.9rem] font-medium sm:w-[initial] sm:flex-nowrap sm:gap-5 transition-colors">
          {links.map((link) => (
            <motion.li
              className="h-3/4 flex items-center justify-center relative text-black dark:text-white yellow:text-white red:text-white blue:text-white"
              key={link.id}
            ...
            >
              ...
              >
                ...
                {link.id === activeSection && (
                  <motion.span
                    className="rounded-full absolute inset-0 -z-10 dark:bg-[#e44646] bg-[#ffcbb4] yellow:bg-[#de2c94] red:bg-[#2182a9] blue:bg-[#288130]"
